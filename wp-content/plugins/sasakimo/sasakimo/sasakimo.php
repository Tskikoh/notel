...
if ( ! class_exists( 'Sasa_Kimo' ) ) :
	class Sasa_Kimo {
		...
		function get_drafts() {
			global $current_user;
			$unpublished_statuses = array( 'pending', 'draft', 'future', 'private' , 'inherit');
		...
		function friendly_delta( $s ) {
			...
			$names = array();
			if ( isset( $res[0] ) ) {
				$names[] = sprintf( _n( '%d分', '%d分', $res[0], 'sasakimo' ), $res[0] );
			}
			if ( isset( $res[1] ) ) {
				$names[] = sprintf( _n( '%d時間', '%d時間', $res[1], 'sasakimo' ), $res[1] );
			}
			if ( isset( $res[2] ) ) {
				$names[] = sprintf( _n( '%d日', '%d日', $res[2], 'sasakimo' ), $res[2] );
			}
